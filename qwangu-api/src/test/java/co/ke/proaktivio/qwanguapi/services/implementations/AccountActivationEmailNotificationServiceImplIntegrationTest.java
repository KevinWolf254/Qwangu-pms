...
class AccountActivationEmailNotificationServiceImplIntegrationTest {
    @Autowired
    ...
    @Autowired
    private AccountActivationEmailNotificationService underTest;
    ...
    void create_returnsEmailNotification_whenSuccessful() {
        ...
        StepVerifier
        	...
        	.expectNextMatches(email -> !email.getId().isEmpty()
        			...
        			&& email.getSubject().equals("Account Activation")
        			&& email.getTemplate().equals(fmpc.getTemplates().get(1).getName())
        			&& email.getTemplateModel().get("companyUrl").equals(cpc.getUrl())
        			&& email.getTemplateModel().get("accountActivationUrl").equals(activationUrl)
        			...
        			&& email.getResources().get("passwordImage").equals(fmpc.getTemplates().get(0).getResources().get(1))
