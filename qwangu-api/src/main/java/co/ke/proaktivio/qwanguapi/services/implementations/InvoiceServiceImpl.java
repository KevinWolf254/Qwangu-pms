...
public class InvoiceServiceImpl implements InvoiceService {
						...
						.doOnSuccess(result -> log.debug("Found: {}", result)).flatMap(previousInvoice -> {
							...
							return findUnitById(unitId)
									.doOnSuccess(result -> log.debug("Found: {}", result)).map(unit -> {
										...
										invoice.setNumber(
												invoice.generateInvoiceNumber.apply(previousInvoice, occupation));
										invoice.setOccupationId(occupationId);
										if (type.equals(Type.RENT_ADVANCE)) {
											BigDecimal totalRentAdvance = unit.getRentPerMonth()
													.multiply(BigDecimal.valueOf(unit.getAdvanceInMonths()));
											...
											BigDecimal rentToPay = unit.getRentPerMonth()
													.multiply(BigDecimal.valueOf(proRate))
											...
											penalty.put("PENALTY", unit.getRentPerMonth()
													.multiply(BigDecimal.valueOf(
															(double) rentPropertiesConfig.getPenaltyPercentageOfRent()
																	/ (double) 100))
													.setScale(0, RoundingMode.CEILING));
	...
	private Mono<Occupation> findOccupationById(String occupationId) {
		return template.findById(occupationId, Occupation.class).switchIfEmpty(Mono
				.error(new CustomBadRequestException("Occupation with id %s does not exist!".formatted(occupationId))));
	}

	private Mono<Unit> findUnitById(String unitId) {
		return template.findById(unitId, Unit.class).switchIfEmpty(Mono
				.error(new CustomBadRequestException("Unit with id %s does not exist!".formatted(unitId))));
	}

	private Mono<Invoice> findLatestInvoiceByOccupationId(String occupationId) {
		return template
				.findOne(new Query().addCriteria(Criteria.where("occupationId").is(occupationId))
