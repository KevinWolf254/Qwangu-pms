...
//            .filter(notice -> params.getOccupationDate().isAfter(notice.getVacatingOn()))
//            .switchIfEmpty(Mono.error(new CustomBadRequestException("Can not occupy before current tenant's vacating date!")))
//            .filter(notice -> params.getOccupationDate().minusDays(14).isEqual(notice.getVacatingOn()) ||
//                    params.getOccupationDate().minusDays(14).isBefore(notice.getVacatingOn()))
